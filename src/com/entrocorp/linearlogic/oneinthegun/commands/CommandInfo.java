...
public class CommandInfo extends OITGArenaCommand {
    ...
    public void run() {
        sender.sendMessage(ChatColor.GRAY + "[]" + ChatColor.RED + "===" + ChatColor.GRAY + "[" + ChatColor.DARK_RED +
                ...
                arena.getPlayerCount() + ChatColor.GRAY + "/" + ChatColor.LIGHT_PURPLE + (arena.getPlayerLimit() == -1 ?
                "infinity" : arena.getPlayerLimit()) + ChatColor.GRAY + "\nPlayers needed to start: " + ChatColor.LIGHT_PURPLE +
                arena.getStartCount() + ChatColor.GRAY + "\nTime limit: " + ChatColor.LIGHT_PURPLE + arena.getTimeLimitFormatted() +
                ChatColor.GRAY + (arena.isIngame() ? " (" + ChatColor.LIGHT_PURPLE + arena.getTimerFormatted() + ChatColor.GRAY +
                " remaining in current round)\n" : "\n") + "Kill limit: " + ChatColor.LIGHT_PURPLE + (arena.getKillLimit() == -1 ?
                "none" : arena.getKillLimit()) + ChatColor.GRAY + "\n\nBlock placing: " + parseEnabled(arena.isBlockPlacingAllowed()) +
                "\nBlock breaking: " + parseEnabled(arena.isBlockBreakingAllowed()) + "\nHealth regeneration: " +
                parseEnabled(arena.isHealthRegenAllowed()) + "\nHunger: " + parseEnabled(arena.isHungerAllowed()) + "\nItem dropping: " +
                parseEnabled(arena.isItemDroppingAllowed()) + "\nItem pickup: " + parseEnabled(arena.isItemPickupAllowed()) +
